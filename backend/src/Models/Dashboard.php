...
class Dashboard
{
    ...
    {
        return [
            'pedidos_pendientes' => $this->countPedidos(['estado' => 'pendiente']),
            'pedidos_entregados' => $this->countPedidos(['estado' => 'entregado']),
            'pedidos_del_dia' => $this->countPedidos(['hoy' => true]),
            'repartidores_activos' => $this->countRepartidoresActivos(),
            'pedidos_en_ruta' => $this->countPedidos(['estado' => 'en_ruta']),
            'pedidos_cancelados' => $this->countPedidos(['estado' => 'cancelado']),
        ];
    }
    ...
    public function reportes(): array
    {
        ...
        $ultimosEntregados = $this->pdo->query(
        ...
        $ultimosCancelados = $this->pdo->query(
        ...
        return [
            ...
            'ultimos_entregados' => $ultimosEntregados,
            'ultimos_cancelados' => $ultimosCancelados,
        ];
    }
    ...
    private function countRepartidoresActivos(): int
    {
        $stmt = $this->pdo->query(
            "SELECT COUNT(*) FROM usuarios WHERE rol = 'repartidor' AND activo = 1"
        );
