...
class Usuario
{
    ...
    public function getAll(?string $search = null, ?string $rol = null): array
    {
        $sql = 'SELECT id, nombre, email, telefono, rol, activo, created_at, updated_at
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO usuarios (nombre, email, password, telefono, rol, activo, created_at, updated_at)
             VALUES (:nombre, :email, :password, :telefono, :rol, :activo, NOW(), NOW())'
        );
        $stmt->execute([
            'nombre' => $data['nombre'],
            'email' => $data['email'],
